...
class BOJAScrapper(BaseScrapper):
    ...
    def find_disposiciones(url_boletin):
        ...
        try:
            ...
            if not listado_principal:
                listado_principal = soup.find(
                    ["ol", "ul"], class_=["listado_ordenado_boja", "listado_ordenado"]
                )
            if listado_principal:
                ...
                for enlace in enlaces_html:
                    ...
                    enlaces_intermedios = soup_intermedio.find_all(
                        "a", class_="item_html", title=re.compile("Versión HTML CVE")
                    )
                    enlaces_intermedios += soup_intermedio.find_all(
                        "a", title="Ver disposición"
                    )
                    for enlace_final in enlaces_intermedios:
                        enlaces_finales.append(enlace_final.get("href"))
            else:
                raise ScrapperError(
                    "No se encontró el listado ordenado con las clases especificadas."
                )
        except Exception as e:
            raise Exception(f"Error inesperado: {e}")
    ...
    def _get_summary_link_from_date(self, fecha_busqueda):
        ...
        try:
            ...
            for tabla in tablas_calendario:
                ...
                mes_año_match = re.search(
                    r"Boletines del mes de (\w+) de (\d{4})", summary_text
                )
                if mes_año_match:
                    ...
                    for enlace in enlaces:
                        ...
                        if fecha_iso == fecha_busqueda.strftime("%Y-%m-%d"):
                            if BOJAScrapper.check_extraordinary_boja(href):
                                urls_bojas = (
                                    BOJAScrapper.extract_bojas_from_extraordinary(href)
                                )
    ...
    def download_day(self, day: date) -> tp.List[BOJAMetadataDocument]:
        ...
        try:
            ...
            for (
                ...
                lista_boletines
            ):  # Boletines. Si hay boletin extraordinario esto será 2
    ...
    def download_document(self, url: str) -> BOJAMetadataDocument:
        ...
        soup = HTTPRequester.get_soup(url)
        try:
            acceso_restringido = soup.find(
                "h1", class_="title", string="Texto de acceso restringido"
            )
            if acceso_restringido:
                return None
            cuerpo = soup.find(
                id="cuerpo", class_="grid_11 contenidos_nivel3 boja_disposicion"
            )
            cabecera = soup.find(
                class_="punteado_izquierda cabecera_detalle_disposicion"
            )
            if not cabecera or not cuerpo:
                raise ScrapperError(
                    "No se pudo encontrar la cabecera o el cuerpo del documento"
                )
            h2 = cabecera.find("h2")
            h5 = cabecera.find("h5")
            h3 = cabecera.find("h3")
            ...
            metadata_doc = BOJAMetadataDocument(
                **{
                    "filepath": fn.name,
