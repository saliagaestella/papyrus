...
def _extract_metadata(doc: dict) -> tp.Dict:
    ...
    try:
        fecha_disposicion = datetime.strptime(
            doc["Fechadisposicion"], "%Y%m%d"
        ).strftime("%Y-%m-%d")
...
class BOAScrapper(BaseScrapper):
    ...
    def download_day(self, day: date) -> tp.List[BOAMetadataDocument]:
        ...
        try:
            ...
            if (
                '<span class="titulo">No se han recuperado documentos</span>'
                in raw_result
            ):
            ...
            raise Exception(
                f"Error de red o HTTP al intentar acceder a {self.base_url}: {e}"
            )
    ...
    def download_document(self, url: str) -> MetadataDocument:
        ...
        with tempfile.NamedTemporaryFile("w", delete=False, encoding="utf-8") as fn:
            fn.write(clean_text)
        try:
            metadata_doc = BOAMetadataDocument(
                filepath=fn.name, **_extract_metadata(doc)
            )
        except:
            raise ScrapperError(
                "No se pudo encontrar alguno de los elementos requeridos."
            )
