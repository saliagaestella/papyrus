...
def _extract_span_text(row: BeautifulSoup, regex: str) -> str:
    ...
    return (
        span_element.next_sibling.strip()
...
def _extract_metadata(soup) -> tp.Dict:
    ...
    if fecha_publicacion := _extract_span_text(soup, r"Fecha Pub:"):
        fecha_publicacion = datetime.strptime(fecha_publicacion, "%d/%m/%Y").strftime(
            "%Y-%m-%d"
        )
        metadata_dict["fecha_publicacion"] = fecha_publicacion
        metadata_dict["fecha_disposicion"] = fecha_publicacion
        metadata_dict["anio"] = str(
            datetime.strptime(fecha_publicacion, "%Y-%m-%d").year
        )
        metadata_dict["mes"] = str(
            datetime.strptime(fecha_publicacion, "%Y-%m-%d").month
        )
...
class BOPZScrapper(BaseScrapper):
    def download_day(self, day: date) -> tp.List[BOPZMetadataDocument]:
        ...
        metadata_documents = []
        try:
            id_links = _list_links_day(
                "http://bop.dpz.es/BOPZ/portalBuscarEdictos.do", day_str
            )
    ...
    def download_document(
        self, url: str, metadata: BeautifulSoup
        ...
        with tempfile.NamedTemporaryFile("w", delete=False) as fn:
            text = soup.find("div", class_="medium-12 panel").get_text(
