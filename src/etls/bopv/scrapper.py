        ...
        try:
            ...
            for block in txt_blocks:
                ...
                if not titulo or not titulo.find("a"):
                    raise ScrapperError(
                        "No se pudo encontrar el título o el enlace en uno de los bloques."
                    )
            ...
            raise Exception(
                f"Error de red o HTTP al intentar acceder a {summary_link}: {e}"
            )
    ...
    def download_document(self, url: str) -> BOPVMetadataDocument:
        ...
        try:
            ...
            if not organismo_tag or not content_block or not pdf_link_tag:
                raise ScrapperError(
                    "No se pudo encontrar algunos de los elementos requeridos."
                )
            ...
            paragraphs = content_block.find_all(
                "p",
                class_=re.compile(
                    r"BOPV(Detalle|Titulo|FirmaLugFec|FirmaPuesto|FirmaNombre)"
                ),
            )
            content_paragraphs = [p.get_text(strip=True) for p in paragraphs]
            additional_elements = content_block.find_all(
                ["h5", "div"],
                class_=re.compile(r"BOPV(Titulo|FirmaLugFec|FirmaPuesto|FirmaNombre)"),
            )
            content_additional = [
                elem.get_text(strip=True) for elem in additional_elements
            ]
            ...
            metadata_doc = BOPVMetadataDocument(
                **{
                    "filepath": fn.name,
